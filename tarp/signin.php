...
$_POST['last_name'] = trim($_POST['last_name']);
	try {
		...
		$check = 0;
		$e_check = mysqli_query($connection,"SELECT email FROM `user` WHERE email='$u_email'");
			...
			if ($check == 0 ) {
				if ($email_check == 0) {
					if (strlen($_POST['password']) >1 ) {
						...
						if ($_POST['email']) {
							$confirmCode==0;
						$result = mysqli_query($connection,"INSERT INTO user (firstName,lastName,email,mobile,address,password,confirmCode) VALUES ('$_POST[first_name]','$_POST[last_name]','$_POST[email]','$_POST[mobile]','$_POST[signupaddress]','$_POST[password]','$confirmCode')");
